...
public abstract class GameLogic {
    ...
    public boolean endGame(){
        ...
        if(contador == 7){
            ...
            if(pPoints[0] > pPoints[1]){
                ...
                this.player1.setWins(player1.getWins() + 1);
            }else if(pPoints[1] > pPoints[0]){
                System.out.println("Vitoria do Player 2!");
                this.player1.setWins(player2.getWins() + 1);
            }else{
    ...
    public int botPlays(Player player,Player bot){
        ...
        jogada = checkPlay(bot,jogada);
        jogada = checkPlay(player,jogada);
        
        return jogada;
        
    }
    
    public int checkPlay(Player p, int jogada){
        
        int scoreAtual = 0;
        
        if(p.getBot() == true){
            scoreAtual = pPoints[1] - pPoints[0];
        }else if(p.getBot() == false){
            scoreAtual = pPoints[0] - pPoints[1];
        }
        
        for (int botX = 0; botX < 7; botX++) {
            boolean disapproved;
            
            int botY = choosePlace(botX);
            disapproved = putPiece(botY,botX,p);
            ...
            if(disapproved == false){
                ...
                tab.getPeca(botY,botX).setExiste(false);
                tab.getPeca(botY,botX).setPlayer(null);
                tab.setTabuleiroJogo(0,botX,botY); //TEMPORÁRIO
